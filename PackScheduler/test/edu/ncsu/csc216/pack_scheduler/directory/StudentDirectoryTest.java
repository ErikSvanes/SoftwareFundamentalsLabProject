...
public class StudentDirectoryTest {
	...
	public void testAddStudentValid() {
		...
		sd.addStudent(LAST_NAME, FIRST_NAME, ID, EMAIL, PASSWORD, PASSWORD, 2);
		sd.addStudent(LAST_NAME, FIRST_NAME, ID, EMAIL, PASSWORD, PASSWORD, 19);
	}
	...
	public void testAddStudentInvalid() {
		// Password tests
		Exception e1 = assertThrows(IllegalArgumentException.class,
				() -> new StudentDirectory().addStudent(FIRST_NAME, LAST_NAME, ID, EMAIL, null, PASSWORD, MAX_CREDITS));
		assertEquals("Invalid password", e1.getMessage());
		Exception e2 = assertThrows(IllegalArgumentException.class,
				() -> new StudentDirectory().addStudent(FIRST_NAME, LAST_NAME, ID, EMAIL, PASSWORD, null, MAX_CREDITS));
		assertEquals("Invalid password", e2.getMessage());
		Exception e3 = assertThrows(IllegalArgumentException.class,
				() -> new StudentDirectory().addStudent(FIRST_NAME, LAST_NAME, ID, EMAIL, "", PASSWORD, MAX_CREDITS));
		assertEquals("Invalid password", e3.getMessage());
		Exception e4 = assertThrows(IllegalArgumentException.class,
				() -> new StudentDirectory().addStudent(FIRST_NAME, LAST_NAME, ID, EMAIL, PASSWORD, "", MAX_CREDITS));
		assertEquals("Invalid password", e4.getMessage());
		Exception e5 = assertThrows(IllegalArgumentException.class,
				() -> new StudentDirectory().addStudent(FIRST_NAME, LAST_NAME, ID, EMAIL, PASSWORD, "wp", MAX_CREDITS));
		assertEquals("Passwords do not match", e5.getMessage());
		// Test for if student IDs repeat
		StudentDirectory sd = new StudentDirectory();
		// Add students
		sd.loadStudentsFromFile(validTestFile);
		// cschwartz is already in the student directory
		sd.addStudent(FIRST_NAME, LAST_NAME, "cschwartz", EMAIL, PASSWORD, PASSWORD, MAX_CREDITS);
	}
	 ...
	 * @throws Exception If there is an error saving it.
	 */
	@Test
	public void testSaveStudentDirectory() throws Exception {
		StudentDirectory sd = new StudentDirectory();
			//Add a student
